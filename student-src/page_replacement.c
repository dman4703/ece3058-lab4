...
pfn_t free_frame(void) {
    // check if victim frame is currently mapped, Frame_table[victim_pfn]
    // if not mapped, return the victim page frame number
    // if victim page is mapped, get its process's page table entry with frame_table[victim_pfn]. If pte is dirty, increment Stats.writebacks, Evict with swap_write() (pass in `mem + victim_pfn * PAGE_SIZE` for src param), clear dirty bit. Clear mapped bit of frame table entry. Return victim_pfn 
